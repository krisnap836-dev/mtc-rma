...
const SEED_DOWNTIMES = [
  { id: "s1", date: "2025-06-01", plant: "PLANT 1", machine: "Mesin Press A", category: "Mekanik", duration: 3.5, description: "Bearing aus", status: "Selesai", technician: "Budi S." },
  { id: "s2", date: "2025-06-02", plant: "PLANT 2", machine: "Kompresor B", category: "Pneumatik", duration: 2.0, description: "Kebocoran selang", status: "Selesai", technician: "Anton W." },
  { id: "s3", date: "2025-06-03", plant: "PLANT 3", machine: "Cylinder Hidrolik C", category: "Hidrolik", duration: 5.5, description: "Seal bocor", status: "Selesai", technician: "Sari M." },
  { id: "s4", date: "2025-06-04", plant: "PLANT 4", machine: "Panel Kontrol D", category: "Elektrik", duration: 1.5, description: "Kontaktor rusak", status: "Selesai", technician: "Dian R." },
  { id: "s5", date: "2025-06-05", plant: "PLANT 6", machine: "Mesin CNC E", category: "Mekanik", duration: 4.0, description: "Spindle error", status: "Selesai", technician: "Budi S." },
  { id: "s6", date: "2025-06-06", plant: "PLANT 7", machine: "Valve Pneumatik F", category: "Pneumatik", duration: 1.0, description: "Solenoid valve mati", status: "Selesai", technician: "Anton W." },
  { id: "s7", date: "2025-06-07", plant: "PLANT 1 KRW", machine: "Pompa Hidrolik G", category: "Hidrolik", duration: 3.0, description: "Pompa overheating", status: "Proses", technician: "Sari M." },
  { id: "s8", date: "2025-06-08", plant: "PLANT 2 KRW", machine: "Motor Listrik H", category: "Elektrik", duration: 2.5, description: "Winding terbakar", status: "Pending", technician: "Dian R." },
];
const SEED_PARTS = [
...
function Dashboard({ downtimes, theme }) {
  ...
  const totalJam = dtPlant.reduce((s, d) => s + Number(d.duration || 0), 0).toFixed(1);

  const catData = CATEGORIES.map(c => ({
    name: c,
    jam:  Number(dtPlant.filter(d => d.category === c).reduce((s, d) => s + Number(d.duration || 0), 0).toFixed(1)),
    count:dtPlant.filter(d => d.category === c).length,
  }));

  const plantData = PLANTS.map(p => ({
    name: p,
    jam:  Number(dtPlantOnly.filter(d => d.plant === p && (!selectedMonth || d.date?.slice(5,7)===selectedMonth)).reduce((s, d) => s + Number(d.duration || 0), 0).toFixed(1)),
    count:dtPlantOnly.filter(d => d.plant === p && (!selectedMonth || d.date?.slice(5,7)===selectedMonth)).length,
  })).filter(p => p.jam > 0 || p.count > 0);

  // ─── PARETO DATA (berdasarkan kategori downtime) ───
  const paretoTotal = dtPlant.reduce((s, d) => s + Number(d.duration || 0), 0);
  const paretoSorted = [...catData].filter(c => c.jam > 0).sort((a,b) => b.jam - a.jam);
  let cumSum = 0;
  const paretoData = paretoSorted.map(c => {
    cumSum += c.jam;
    return { name: c.name, jam: c.jam, kumulatif: paretoTotal ? Number(((cumSum/paretoTotal)*100).toFixed(1)) : 0 };
  });
  ...
  const pieData = CATEGORIES.map(c => ({
    name:  c,
    value: Number(dtPlant.filter(d => d.category === c).reduce((s, d) => s + Number(d.duration || 0), 0).toFixed(1)),
  })).filter(p => p.value > 0);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:16 }}>
        <StatCard label="Total Downtime"   value={`${totalJam} Jam`}        sub={`${total} kejadian`}  color="#f59e0b" icon={Timer} theme={T}/>
      ...
      <div style={cardStyle}>
        <h3 style={{ color: T.text, fontWeight:600, fontSize:13, margin:"0 0 16px" }}><BarChart3 size={15} style={{display:"inline",verticalAlign:"-3px",marginRight:6}}/>Total Jam Downtime per Kategori</h3>
      ...
      <div style={cardStyle}>
        ...
        <p style={{ color: T.textMuted, fontSize:12, margin:"0 0 16px" }}>Diurutkan dari kontributor jam downtime terbesar, dengan garis persentase kumulatif (aturan 80/20)</p>
        {paretoData.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={paretoData} barSize={50}>
              <CartesianGrid strokeDasharray="3 3" stroke={T.tableBorder}/>
              <XAxis dataKey="name" tick={{ fill: T.textSub, fontSize:12 }}/>
              <YAxis yAxisId="left" tick={{ fill: T.textSub, fontSize:12 }} label={{ value:"Jam", angle:-90, position:"insideLeft", fill: T.textSub, fontSize:11 }}/>
              ...
              <Tooltip {...tt} formatter={(v, n) => n==="kumulatif" ? [`${v}%`, "Kumulatif"] : [`${v} jam`, "Total Jam"]}/>
              <Legend wrapperStyle={{ color: T.textSub, fontSize:12 }}/>
              <Bar yAxisId="left" dataKey="jam" name="Jam">{paretoData.map(e => <Cell key={e.name} fill={CAT_COLORS[e.name]||"#f59e0b"}/>)}</Bar>
      ...
      <div style={cardStyle}>
        <h3 style={{ color: T.text, fontWeight:600, fontSize:13, margin:"0 0 16px" }}><BarChart3 size={15} style={{display:"inline",verticalAlign:"-3px",marginRight:6}}/>Total Jam Downtime per Plant{selectedPlant ? ` (${selectedPlant})` : ""}</h3>
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={selectedPlant ? plantData.filter(p => p.name === selectedPlant) : plantData} barSize={30}>
            ...
            <Tooltip {...tt} formatter={(v, n) => [n==="jam"?`${v} jam`:`${v} kejadian`, n==="jam"?"Total Jam":"Jumlah"]}/>
            <Legend wrapperStyle={{ color: T.textSub, fontSize:12 }}/>
            <Bar dataKey="jam" name="Jam" fill="#f59e0b" radius={[6,6,0,0]}/>
            <Bar dataKey="count" name="Kejadian" fill={T.tableBorder} radius={[6,6,0,0]}/>
          </BarChart>
        </ResponsiveContainer>
      </div>
...
function DowntimePage({ downtimes, setDowntimes, toast, theme }) {
  ...
  function exportExcel() {
    const data = downtimes.map(d => ({ Tanggal:d.date, Plant:d.plant||"", Mesin:d.machine, Kategori:d.category, "Durasi (jam)":d.duration, Deskripsi:d.description, Status:d.status, Teknisi:d.technician }));
  ...
  function downloadTemplate() {
    const data = [
      { Tanggal:"2025-06-01", Plant:"PLANT 1", Mesin:"Mesin Press A", Kategori:"Mekanik", "Durasi (jam)":3.5, Deskripsi:"Bearing aus", Status:"Selesai", Teknisi:"Budi S." },
      { Tanggal:"2025-06-02", Plant:"PLANT 2", Mesin:"Kompresor B",   Kategori:"Pneumatik","Durasi (jam)":2,   Deskripsi:"Contoh isi", Status:"Pending", Teknisi:"Anton W." },
    ];
    const wb = XLSX.utils.book_new();
