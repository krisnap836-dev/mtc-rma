...
const PLANTS = ["PLANT 1","PLANT 2","PLANT 3","PLANT 4","PLANT 6","PLANT 7","PLANT 1 KRW","PLANT 2 KRW","PLANT 3 KRW","PLANT 10"];
...
function Dashboard({ downtimes, theme }) {
  ...
  const dtPlant = selectedPlant ? downtimes.filter(d => d.plant === selectedPlant) : downtimes;
  ...
  const plantData = PLANTS.map(p => ({
    name: p,
    jam:  Number(downtimes.filter(d => d.plant === p).reduce((s, d) => s + Number(d.duration || 0), 0).toFixed(1)),
    count:downtimes.filter(d => d.plant === p).length,
  })).filter(p => p.jam > 0 || p.count > 0);

  const MONTH_NAMES = ["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"];
  const years = Array.from(new Set(dtPlant.map(d => d.date?.slice(0,4)).filter(Boolean)));
  ...
  const trendData = MONTH_NAMES.map((m, i) => {
    ...
    const items = dtPlant.filter(d => d.date?.slice(0,4) === selectedYear && d.date?.slice(5,7) === mm);
    return {
      day: m,
