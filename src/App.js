...
import { LayoutDashboard, Clock, Wrench, ShoppingCart, ClipboardList, CheckSquare, Settings as SettingsIcon, Sun, Moon, Menu, Cog, Wind, Droplet, Zap, Save, Trash2, Download, Upload, FileText, AlertTriangle, CheckCircle2, Search, X, Inbox, TrendingUp, BarChart3, PieChartIcon, Plus, Timer, ListChecks, CheckCheck, AlarmClock, FileEdit, Send as SendIcon, ThumbsUp, PackageCheck, FilePlus2, Sunrise, Sunset, MoonStar, ShieldCheck, ClipboardCheck, Ruler, Sparkles, User, LogOut, UserPlus, Lock, Eye, EyeOff, Users } from "lucide-react";

const PAGE_LIST = [
  { id: "dashboard",  label: "Dashboard"     },
  { id: "downtime",   label: "Downtime"      },
  { id: "spareparts", label: "Spare Part"    },
  { id: "orders",     label: "Order Part"    },
  { id: "activity",   label: "Daily Activity"},
  { id: "todo",       label: "To-Do List"    },
  { id: "assetmesin", label: "Asset Mesin"   },
  { id: "settings",   label: "Settings"      },
];
const DEFAULT_ADMIN_PERMS = { dashboard:"edit", downtime:"edit", spareparts:"edit", orders:"edit", activity:"edit", todo:"edit", assetmesin:"edit", settings:"edit" };
const DEFAULT_USER_PERMS  = { dashboard:"view", downtime:"edit", spareparts:"view", orders:"none", activity:"edit", todo:"none", assetmesin:"view", settings:"none" };

const SEED_USERS = [
  { id:"u1", username:"admin", password:"admin123", role:"admin", name:"Administrator",   permissions: DEFAULT_ADMIN_PERMS },
  { id:"u2", username:"user",  password:"user123",  role:"user",  name:"User Lapangan",   permissions: DEFAULT_USER_PERMS  },
];
...
const SEED_ACTIVITIES = [
  { id: "a1", date: "2025-06-08", plant: "PLANT 1", shift: "Pagi", type: "Preventive", machine: "Mesin Press A", description: "Pelumasan rutin, cek belt dan bearing", status: "Selesai", duration: 120 },
  { id: "a2", date: "2025-06-08", plant: "PLANT 2", shift: "Siang", type: "Preventive", machine: "Kompresor B", description: "Cek tekanan, drain kondensasi, kalibrasi", status: "Selesai", duration: 60 },
  { id: "a3", date: "2025-06-07", plant: "PLANT 1 KRW", shift: "Pagi", type: "Corrective", machine: "Pompa Hidrolik G", description: "Ganti seal, flush sistem, test pressure", status: "Proses", duration: 240 },
];
...
// pages visible to a user based on permissions (≠ none)
function getVisiblePages(user) {
  if (!user) return [];
  const perms = (user.role === "admin") ? DEFAULT_ADMIN_PERMS : (user.permissions || DEFAULT_USER_PERMS);
  return PAGE_LIST.filter(p => (perms[p.id] || "none") !== "none");
}
function getPerm(user, pageId) {
  if (!user) return "none";
  if (user.role === "admin") return "edit";
  return (user.permissions || DEFAULT_USER_PERMS)[pageId] || "none";
}
const PAGE_ICONS = {
  dashboard: LayoutDashboard, downtime: Clock, spareparts: Wrench,
  orders: ShoppingCart, activity: ClipboardList, todo: CheckSquare, assetmesin: Cog, settings: SettingsIcon,
};
...
// ─── DOWNTIME PAGE ────────────────────────────────────────────────────────────
function DowntimePage({ downtimes, setDowntimes, toast, theme, perm }) {
  ...
  function exportExcel() {
    const data = downtimes.map(d => ({ Tanggal:d.date, Plant:d.plant||"", Mesin:d.machine, Kategori:d.category, "Durasi (menit)":d.duration, Deskripsi:d.description, Status:d.status }));
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"space-between", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {canEdit && <BtnSecondary onClick={downloadTemplate} theme={T}><FileText size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>Template</BtnSecondary>}
          {canEdit && <BtnSecondary onClick={() => fileRef.current.click()} theme={T}><Upload size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>Import Excel</BtnSecondary>}
          ...
          {canEdit && <DeleteAllButton items={downtimes} colName="downtimes" setItems={setDowntimes} toast={toast}/>}
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
            <thead style={{ background: T.tableHead }}>
              <tr>{(canEdit ? ["Tanggal","Plant","Mesin","Kategori","Durasi","Deskripsi","   ","Status","Aksi"] : ["Tanggal","Plant","Mesin","Kategori","Durasi","Deskripsi","   ","Status"]).map(h => (
                <th key={h} style={{ textAlign:"left", padding:"12px 18px", color: T.textSub, fontSize:11, fontWeight:600, whiteSpace:"nowrap" }}>{h}</th>
              ))}</tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={canEdit ? 9 : 8} style={{ padding:"40px 16px", textAlign:"center", color: T.textMuted }}>
                  <Inbox size={28} style={{margin:"0 auto 8px", opacity:0.5}}/>Tidak ada data
                </td></tr>
...
// ─── SPARE PARTS ─────────────────────────────────────────────────────────────
function SparePartsPage({ parts, setParts, toast, theme, perm }) {
  const T = theme;
  const canEdit = perm === "edit";
  const [showForm, setShowForm] = useState(false);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"space-between", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {canEdit && <DeleteAllButton items={parts} colName="spareparts" setItems={setParts} toast={toast}/>}
          {canEdit && <BtnPrimary onClick={() => setShowForm(true)}>+ Tambah Part</BtnPrimary>}
        </div>
      </div>
        ...
        {parts.map(p => (
          <div key={p.id} style={{ ...cardStyle, border:`1px solid ${p.stock <= p.minStock ? "rgba(239,68,68,0.4)" : T.cardBorder}` }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
              {canEdit && <button onClick={() => updateStock(p.id, -1)} style={{ width:32, height:32, borderRadius:8, border:`1px solid ${T.inputBorder}`, background: T.input, color: T.text, fontSize:16, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>−</button>}
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", paddingTop:10, borderTop:`1px solid ${T.divider}` }}>
              ...
              {canEdit && <button onClick={() => remove(p.id)} style={{ padding:"3px 10px", borderRadius:6, fontSize:12, color:"#ef4444", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", cursor:"pointer" }}><Trash2 size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:4}}/>Hapus</button>}
            </div>
          </div>
        ))}
      </div>
...
// ─── ORDER PAGE ───────────────────────────────────────────────────────────────
function OrderPage({ orders, setOrders, toast, theme, perm }) {
  const T = theme;
  const canEdit = perm === "edit";
  const [showForm, setShowForm] = useState(false);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"space-between", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {canEdit && <DeleteAllButton items={orders} colName="orders" setItems={setOrders} toast={toast}/>}
          {canEdit && <BtnPrimary onClick={() => setShowForm(true)}>+ Buat Order</BtnPrimary>}
        </div>
      </div>
            ...
            <tbody>
              {orders.length === 0 ? (
                ...
                <tr key={o.id} style={{ borderTop:`1px solid ${T.tableBorder}` }}>
                  ...
                  <td style={{ padding:"12px 16px" }}>
                    {canEdit && <button onClick={() => remove(o.id)} style={{ padding:"4px 10px", borderRadius:6, fontSize:12, color:"#ef4444", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", cursor:"pointer" }}><Trash2 size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:4}}/>Hapus</button>}
                  </td>
                </tr>
              ))}
            </tbody>
...
// ─── DAILY ACTIVITY ───────────────────────────────────────────────────────────
function ActivityPage({ activities, setActivities, toast, theme, perm }) {
  const canEdit = perm === "edit";
  const T = theme;
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [filterMonth, setFilterMonth] = useState(today().slice(0,7));
  const blank = { date:today(), plant:"", shift:"Pagi", type:"Preventive", machine:"", description:"", status:"Proses", duration:"" };
  const [form, setForm] = useState(blank);

  async function submit() {
    if (!form.machine || !form.description) return alert("Mesin dan Deskripsi wajib!");
    
    if (editingId) {
      // Update existing activity
      const updated = { ...form, duration:Number(form.duration) };
      await saveDoc("activities", editingId, updated);
      setActivities(p => p.map(a => sid(a.id, editingId) ? { ...a, ...updated } : a));
      toast("Aktivitas berhasil diperbarui!");
    } else {
      // Create new activity
  ...
  const tIcons    = { Preventive:ShieldCheck, Corrective:Wrench };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"space-between", gap:12 }}>
        <div>
          <h2 style={{ color: T.text, fontWeight:700, fontSize:28, margin:0 }}>Daily Activity</h2>
          <p style={{ color: T.textSub, fontSize:13, margin:"6px 0 0" }}>Log aktivitas maintenance • {activities.length} total • {filtered.length} ditampilkan</p>
        </div>
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          ...
          {canEdit && <DeleteAllButton items={activities} colName="activities" setItems={setActivities} toast={toast}/>}
          <BtnPrimary onClick={() => { setEditingId(null); setForm(blank); setShowForm(true); }}>+ Log Aktivitas</BtnPrimary>
        </div>
      </div>
        ...
        return (
          <div key={shift}>
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
              <div style={{ color: T.text, fontWeight:600, fontSize:14, display:"flex", alignItems:"center", gap:6 }}>{(() => { const SI = shiftIcons[shift]; return <SI size={16}/>; })()}Shift {shift}</div>
              <div style={{ flex:1, height:1, background: T.divider }}/>
              <div style={{ fontSize:12, color: T.textMuted }}>{items.length} aktivitas</div>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              {items.map(a => (
                <div key={a.id} style={{ background: T.card, backdropFilter: T.blur, WebkitBackdropFilter: T.blur, boxShadow: T.shadow, border:`1px solid ${T.cardBorder}`, borderRadius:12, padding:14, display:"flex", gap:14, alignItems:"center", transition:"all 0.3s ease" }}>
                  <div style={{ flexShrink:0, width:40, height:40, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", background: a.type === "Preventive" ? "rgba(16,185,129,0.15)" : "rgba(239,68,68,0.15)", color: a.type === "Preventive" ? "#10b981" : "#ef4444" }}>
                    {(() => { const TI = tIcons[a.type] || ClipboardList; return <TI size={18}/>; })()}
                  </div>
                  
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                      <span style={{ color: T.text, fontWeight:600, fontSize:13 }}>{a.machine}</span>
                      {a.plant && <span style={{ fontSize:10, padding:"2px 8px", borderRadius:999, background:"rgba(245,158,11,0.15)", color:"#f59e0b", fontWeight:500 }}>{a.plant}</span>}
                      <span style={{ fontSize:10, padding:"2px 8px", borderRadius:999, background:"rgba(59,130,246,0.15)", color:"#60a5fa", fontWeight:500 }}>{a.type}</span>
                      <Badge status={a.status}/>
                    </div>
                    <div style={{ color: T.textSub, fontSize:12, marginBottom:3 }}>{a.description}</div>
                    <div style={{ color: T.textMuted, fontSize:11, display:"flex", gap:8 }}>
                      <span>📅 {a.date}</span>
                      {a.duration && <span>⏱ {a.duration} menit</span>}
                    </div>
                  </div>
                  ...
                  {canEdit && (
                    <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                      <button 
                        ...
                        style={{ padding:"6px 12px", borderRadius:8, fontSize:11, color:"#3b82f6", background:"rgba(59,130,246,0.1)", border:"1px solid rgba(59,130,246,0.2)", cursor:"pointer", fontWeight:500, whiteSpace:"nowrap" }}>
                        ✏️ Edit
                      </button>
                      <button 
                        onClick={() => remove(a.id)} 
                        style={{ padding:"6px 12px", borderRadius:8, fontSize:11, color:"#ef4444", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", cursor:"pointer", fontWeight:500, whiteSpace:"nowrap" }}>
                        🗑️ Hapus
                      </button>
                    </div>
                  )}
      ...
      {filtered.length === 0 && (
        <div style={{ background: T.card, backdropFilter: T.blur, WebkitBackdropFilter: T.blur, boxShadow: T.shadow, border:`1px solid ${T.cardBorder}`, borderRadius:16, padding:"48px 16px", textAlign:"center" }}>
          ...
          <div style={{ color: T.textSub, marginBottom:16, fontSize:14 }}>Tidak ada aktivitas pada periode ini</div>
          <BtnPrimary onClick={() => { setEditingId(null); setForm(blank); setShowForm(true); }}>+ Tambah Aktivitas</BtnPrimary>
        </div>
      )}

      {showForm && (
        <Modal title={editingId ? "Edit Daily Activity" : "Log Daily Activity"} onClose={() => { setShowForm(false); setEditingId(null); setForm(blank); }} theme={T}>
          <div className="mms-form-grid">
            <FInput label="Tanggal" type="date" value={form.date} onChange={v => setForm(f=>({...f,date:v}))} required theme={T}/>
          ...
          <div style={{ display:"flex", gap:10, marginTop:20, justifyContent:"flex-end" }}>
            <BtnSecondary onClick={() => { setShowForm(false); setEditingId(null); setForm(blank); }} theme={T}>Batal</BtnSecondary>
            <BtnPrimary onClick={submit}><Save size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>{editingId ? "Update" : "Simpan"}</BtnPrimary>
          </div>
        </Modal>
      )}
    </div>
...
// ─── TODO LIST PAGE ───────────────────────────────────────────────────────────
function TodoPage({ todos, setTodos, toast, theme, perm }) {
  const T = theme;
  const canEdit = perm === "edit";
  const [showForm, setShowForm] = useState(false);
  const [filter, setFilter] = useState("Semua");
  const blank = { title:"", assignee:"", plant:"", priority:"Sedang", dueDate:today(), notes:"" };
  const [form, setForm] = useState(blank);
  ...
  function exportToExcel() {
    ...
    const data = todos.map(t => ({
      "Judul Tugas": t.title,
      "Plant": t.plant || "-",
      "PIC/Penanggung Jawab": t.assignee || "-",
      "Prioritas": t.priority,
      "Deadline": t.dueDate || "-",
      "Status": t.done ? "✅ Selesai" : "⏳ Belum Selesai",
      "Catatan": t.notes || "-",
      "Dibuat": t.createdAt || "-"
    }));
    ...
    XLSX.writeFile(wb, `TODO-List-${today()}.xlsx`);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"space-between", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {canEdit && <DeleteAllButton items={todos} colName="todos" setItems={setTodos} toast={toast}/>}
          <button onClick={exportToExcel} style={{ padding:"8px 16px", borderRadius:8, fontSize:13, fontWeight:600, cursor:"pointer", border:"1px solid #10b981", background:"rgba(16,185,129,0.1)", color:"#10b981", display:"flex", alignItems:"center", gap:6 }}><Download size={16}/> Export Excel</button>
          {canEdit && <BtnPrimary onClick={() => setShowForm(true)}>+ Tambah Tugas</BtnPrimary>}
        </div>
      </div>
        ...
        {filtered.map(t => {
          ...
          return (
            <div key={t.id} style={{ ...cardStyle, padding:16, display:"flex", gap:14, alignItems:"flex-start", opacity: t.done ? 0.6 : 1 }}>
              {canEdit
                ? <button onClick={() => toggleDone(t.id)} style={{ flexShrink:0, width:24, height:24, borderRadius:8, cursor:"pointer", marginTop:2, border:`2px solid ${t.done ? "#10b981" : T.inputBorder}`, background: t.done ? "#10b981" : "transparent", color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 }}>{t.done ? "✓" : ""}</button>
                : <div style={{ flexShrink:0, width:24, height:24, borderRadius:8, marginTop:2, border:`2px solid ${t.done ? "#10b981" : T.inputBorder}`, background: t.done ? "#10b981" : "transparent", color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 }}>{t.done ? "✓" : ""}</div>
              }
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ color: T.textMuted, fontSize:12, display:"flex", gap:12, flexWrap:"wrap" }}>
                  {t.plant && <span>🏭 {t.plant}</span>}
                  {t.assignee && <span>👤 {t.assignee}</span>}
                  {t.dueDate && <span>📅 {t.dueDate}</span>}
                </div>
              </div>
              {canEdit && <button onClick={() => remove(t.id)} style={{ flexShrink:0, padding:"4px 10px", borderRadius:6, fontSize:12, color:"#ef4444", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", cursor:"pointer" }}><Trash2 size={14}/></button>}
            </div>
          );
        })}
      </div>
...
// ─── ASSET MESIN PAGE ────────────────────────────────────────────────────────
function AssetMesinPage({ assets, setAssets, toast, theme, perm }) {
  const T = theme;
  const canEdit = perm === "edit";
  const [showForm, setShowForm] = useState(false);
  const [filter, setFilter] = useState("Semua");
  const [search, setSearch] = useState("");
  const blank = { code:"", name:"", plant:"", location:"", category:"Mekanik", status:"Aktif", serialNumber:"", manufacturer:"", dateAcquired:today(), condition:"Baik", notes:"" };
  const [form, setForm] = useState(blank);
  const [editingId, setEditingId] = useState(null);

  async function submit() {
    if (!form.code || !form.name) return alert("Kode dan Nama Mesin wajib diisi!");
    const item = editingId ? { ...form, id: editingId } : { ...form, id: uid(), createdAt: today() };
    await saveDoc("assetmesin", item.id, item);
    if (editingId) setAssets(p => p.map(a => sid(a.id, editingId) ? item : a));
    else setAssets(p => [...p, item]);
    setShowForm(false); setForm(blank); setEditingId(null);
    toast(editingId ? "Asset diperbarui!" : "Asset berhasil ditambahkan!");
  }

  async function remove(id) {
    if (!window.confirm("Hapus asset ini?")) return;
    await deleteDocument("assetmesin", id);
    setAssets(p => p.filter(a => !sid(a.id, id)));
    toast("Asset dihapus!");
  }

  function openEdit(a) { setEditingId(a.id); setForm({ ...a }); setShowForm(true); }

  const STATUS_COLORS = { Aktif:"#10b981", Maintenance:"#f59e0b", Rusak:"#ef4444", "Tidak Aktif":"#6b7280" };
  const CONDITION_COLORS = { Baik:"#10b981", "Cukup Baik":"#f59e0b", Rusak:"#ef4444" };
  ...
  const filtered = assets.filter(a => {
    ...
    if (filter === "Aktif" && a.status !== "Aktif") return false;
    if (filter === "Maintenance" && a.status !== "Maintenance") return false;
    if (filter === "Rusak" && a.status !== "Rusak") return false;
    return true;
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"space-between", gap:12 }}>
        <div>
          <h2 style={{ color: T.text, fontWeight:700, fontSize:22, margin:0 }}>Asset Mesin</h2>
          <p style={{ color: T.textSub, fontSize:13, margin:"4px 0 0" }}>{total} mesin • {aktif} aktif • {maintenance} maintenance • {rusak} rusak</p>
        </div>
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {canEdit && <DeleteAllButton items={assets} colName="assetmesin" setItems={setAssets} toast={toast}/>}
          {canEdit && <BtnPrimary onClick={() => { setEditingId(null); setForm(blank); setShowForm(true); }}>+ Tambah Mesin</BtnPrimary>}
        </div>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:16 }}>
        <StatCard label="Total Mesin" value={total} sub="Semua asset" color="#3b82f6" icon="🔧" theme={T}/>
        <StatCard label="Aktif" value={aktif} sub={`${total?((aktif/total)*100).toFixed(0):0}%`} color="#10b981" icon={CheckCircle2} theme={T}/>
        <StatCard label="Maintenance" value={maintenance} sub="Sedang diperbaiki" color="#f59e0b" icon={AlertTriangle} theme={T}/>
        <StatCard label="Rusak" value={rusak} sub="Tidak berfungsi" color="#ef4444" icon={Trash2} theme={T}/>
      </div>

      <div style={{ ...cardStyle, padding:16, display:"flex", gap:12, alignItems:"center" }}>
        ...
        <input 
          ...
          style={{ flex:1, background:"transparent", border:"none", color: T.text, outline:"none", fontSize:13 }}
        />
      </div>

      <div style={{ display:"flex", gap:8 }}>
        {["Semua","Aktif","Maintenance","Rusak"].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            padding:"6px 16px", borderRadius:999, fontSize:12, fontWeight:600, cursor:"pointer",
            border:`1px solid ${filter===f ? "#f59e0b" : T.inputBorder}`,
            background: filter===f ? "rgba(245,158,11,0.12)" : T.input,
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
          <thead>
            <tr style={{ borderBottom:`1px solid ${T.tableBorder}` }}>
              <th style={{ padding:12, textAlign:"left", color: T.textSub, fontWeight:600 }}>Kode</th>
              <th style={{ padding:12, textAlign:"left", color: T.textSub, fontWeight:600 }}>Nama Mesin</th>
              <th style={{ padding:12, textAlign:"left", color: T.textSub, fontWeight:600 }}>Plant</th>
              <th style={{ padding:12, textAlign:"left", color: T.textSub, fontWeight:600 }}>Kategori</th>
              <th style={{ padding:12, textAlign:"left", color: T.textSub, fontWeight:600 }}>Status</th>
              <th style={{ padding:12, textAlign:"left", color: T.textSub, fontWeight:600 }}>Kondisi</th>
              {canEdit && <th style={{ padding:12, textAlign:"center", color: T.textSub, fontWeight:600 }}>Aksi</th>}
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={canEdit ? 7 : 6} style={{ padding:24, textAlign:"center", color: T.textSub }}>Tidak ada mesin ditemukan</td>
              </tr>
            ) : (
              filtered.map(a => (
                <tr key={a.id} style={{ borderBottom:`1px solid ${T.tableBorder}`, background: T.tableRow }}>
                  <td style={{ padding:12, color: T.text, fontWeight:600 }}>{a.code}</td>
                  <td style={{ padding:12, color: T.text }}>{a.name}</td>
                  <td style={{ padding:12, color: T.textSub }}>{a.plant}</td>
                  <td style={{ padding:12, color: T.textSub }}>{a.category}</td>
                  <td style={{ padding:12 }}>
                    <span style={{ fontSize:11, padding:"3px 10px", borderRadius:999, background:(STATUS_COLORS[a.status]||"#666")+"22", color:STATUS_COLORS[a.status]||"#999", fontWeight:600 }}>
                      {a.status}
                    </span>
                  </td>
                  <td style={{ padding:12 }}>
                    <span style={{ fontSize:11, padding:"3px 10px", borderRadius:999, background:(CONDITION_COLORS[a.condition]||"#666")+"22", color:CONDITION_COLORS[a.condition]||"#999", fontWeight:600 }}>
                      {a.condition}
                    </span>
                  </td>
                  {canEdit && (
                    <td style={{ padding:12, textAlign:"center", display:"flex", gap:6, justifyContent:"center" }}>
                      <button onClick={() => openEdit(a)} style={{ padding:"4px 10px", borderRadius:6, fontSize:12, color:"#3b82f6", background:"rgba(59,130,246,0.1)", border:"1px solid rgba(59,130,246,0.2)", cursor:"pointer" }}>Edit</button>
                      <button onClick={() => remove(a.id)} style={{ padding:"4px 10px", borderRadius:6, fontSize:12, color:"#ef4444", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", cursor:"pointer" }}><Trash2 size={14}/></button>
                    </td>
                  )}
                </tr>
              ))
      ...
      {showForm && (
        <Modal title={editingId ? "Edit Asset Mesin" : "Tambah Asset Mesin"} onClose={() => { setShowForm(false); setEditingId(null); }} theme={T}>
          <div className="mms-form-grid">
            ...
            <FInput label="Tanggal Diperoleh" type="date" value={form.dateAcquired} onChange={v => setForm(f=>({...f,dateAcquired:v}))} theme={T}/>
            <div className="mms-col-span-2" style={{ gridColumn:"span 2" }}>
              <FInput label="Catatan" type="textarea" value={form.notes} onChange={v => setForm(f=>({...f,notes:v}))} placeholder="Spesifikasi atau catatan penting..." theme={T}/>
            </div>
          </div>
          <div style={{ display:"flex", gap:10, marginTop:20, justifyContent:"flex-end" }}>
            <BtnSecondary onClick={() => { setShowForm(false); setEditingId(null); }} theme={T}>Batal</BtnSecondary>
            <BtnPrimary onClick={submit}><Save size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>{editingId ? "Update" : "Simpan"}</BtnPrimary>
          </div>
        </Modal>
      )}
...
// ─── SETTINGS PAGE ────────────────────────────────────────────────────────────
function SettingsPage({ toast, theme, users, setUsers }) {
  const T = theme;
  ...
  const inputSt   = { background: T.input, border:`1px solid ${T.inputBorder}`, borderRadius:8, padding:"8px 12px", color: T.text, fontSize:13, outline:"none" };
  const [showForm, setShowForm] = useState(false);
  const [editingUser, setEditingUser] = useState(null); // user obj being edited
  const [showPw, setShowPw]     = useState({});
  const blank = { username:"", password:"", name:"", role:"user", permissions: { ...DEFAULT_USER_PERMS } };
  const [form, setForm] = useState(blank);

  function openAdd() { setEditingUser(null); setForm({ ...blank, permissions: { ...DEFAULT_USER_PERMS } }); setShowForm(true); }
  function openEdit(u) { setEditingUser(u); setForm({ ...u, permissions: { ...(u.permissions || DEFAULT_USER_PERMS) } }); setShowForm(true); }

  async function saveUser() {
    if (!form.username || !form.password || !form.name) return alert("Lengkapi semua field!");
    if (!editingUser && users.some(u => u.username.toLowerCase() === form.username.trim().toLowerCase())) return alert("Username sudah dipakai!");
    const item = { ...form, id: editingUser ? editingUser.id : uid() };
    await saveDoc("users", item.id, item);
    if (editingUser) setUsers(p => p.map(u => sid(u.id, editingUser.id) ? item : u));
    else setUsers(p => [...p, item]);
    setForm(blank); setShowForm(false); setEditingUser(null);
    toast(editingUser ? "User diperbarui!" : "User berhasil ditambahkan!");
  }

  async function removeUser(u) {
    if (u.username === "admin") return alert("User admin utama tidak bisa dihapus!");
    if (!window.confirm(`Hapus user "${u.name}" (${u.username})?`)) return;
    await deleteDocument("users", u.id);
    setUsers(p => p.filter(x => !sid(x.id, u.id)));
    toast("User berhasil dihapus!");
  }

  function setPermission(pageId, val) {
    setForm(f => ({ ...f, permissions: { ...f.permissions, [pageId]: val } }));
  }

  const PERM_OPTIONS = [
    { val:"none",  label:"Tidak Bisa Akses", color:"#6b7280" },
    { val:"view",  label:"Lihat Saja",        color:"#3b82f6" },
    { val:"edit",  label:"Edit/Input",         color:"#10b981" },
  ];
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div>
        ...
        <p style={{ color: T.textSub, fontSize:13, margin:"4px 0 0" }}>Kelola user dan hak akses</p>
      </div>

      <div style={cardStyle}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
          <h3 style={{ color: T.text, fontWeight:600, fontSize:15, margin:0 }}><Users size={16} style={{display:"inline",verticalAlign:"-3px",marginRight:6}}/>Kelola User & Hak Akses</h3>
          <BtnPrimary onClick={openAdd}><UserPlus size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>Tambah User</BtnPrimary>
        </div>

        {/* Legend */}
        <div style={{ display:"flex", gap:12, marginBottom:16, flexWrap:"wrap" }}>
          {PERM_OPTIONS.map(o => (
            <div key={o.val} style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color: T.textSub }}>
        ...
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
            <thead>
              <tr style={{ borderBottom:`1px solid ${T.tableBorder}` }}>
                <th style={{ textAlign:"left", padding:"8px 12px 8px 0", color: T.textMuted, fontWeight:600 }}>Nama / Username</th>
                <th style={{ textAlign:"left", padding:"8px 12px", color: T.textMuted, fontWeight:600 }}>Password</th>
                {PAGE_LIST.map(p => (
                  <th key={p.id} style={{ textAlign:"center", padding:"8px 8px", color: T.textMuted, fontWeight:600, fontSize:11, whiteSpace:"nowrap" }}>{p.label}</th>
                ))}
                <th style={{ padding:"8px 0 8px 8px", color: T.textMuted, fontWeight:600 }}>Aksi</th>
              </tr>
            </thead>
            <tbody>
              {users.map(u => {
                const perms = u.role === "admin" ? DEFAULT_ADMIN_PERMS : (u.permissions || DEFAULT_USER_PERMS);
                return (
                  <tr key={u.id} style={{ borderTop:`1px solid ${T.tableBorder}` }}>
                    <td style={{ padding:"10px 12px 10px 0" }}>
                      <div style={{ color: T.text, fontWeight:600 }}>{u.name}</div>
                      <div style={{ color: T.textMuted, fontSize:11 }}>@{u.username} · <span style={{ color: u.role==="admin" ? "#f59e0b" : "#60a5fa" }}>{u.role}</span></div>
                    </td>
                    <td style={{ padding:"10px 12px" }}>
                      <span style={{ display:"flex", alignItems:"center", gap:6, color: T.textSub }}>
                        {showPw[u.id] ? u.password : "••••••"}
                        <button onClick={() => setShowPw(s => ({ ...s, [u.id]: !s[u.id] }))} style={{ background:"none", border:"none", cursor:"pointer", color: T.textMuted, display:"flex" }}>
                          {showPw[u.id] ? <EyeOff size={12}/> : <Eye size={12}/>}
                        </button>
                      </span>
                    </td>
                    {PAGE_LIST.map(p => {
                      const pv = perms[p.id] || "none";
                      const col = PERM_OPTIONS.find(o => o.val === pv)?.color || "#6b7280";
                      return (
                        <td key={p.id} style={{ padding:"10px 8px", textAlign:"center" }}>
                          <div style={{ width:14, height:14, borderRadius:4, background: col, margin:"0 auto", title: pv }}/>
                        </td>
                      );
                    })}
                    <td style={{ padding:"10px 0 10px 8px", whiteSpace:"nowrap" }}>
                      <button onClick={() => openEdit(u)} style={{ padding:"4px 10px", borderRadius:6, fontSize:12, color:"#3b82f6", background:"rgba(59,130,246,0.1)", border:"1px solid rgba(59,130,246,0.2)", cursor:"pointer", marginRight:4 }}><FileEdit size={13} style={{display:"inline",verticalAlign:"-2px",marginRight:3}}/>Edit</button>
                      {u.username !== "admin" && <button onClick={() => removeUser(u)} style={{ padding:"4px 10px", borderRadius:6, fontSize:12, color:"#ef4444", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", cursor:"pointer" }}><Trash2 size={13} style={{display:"inline",verticalAlign:"-2px",marginRight:3}}/>Hapus</button>}
                    </td>
                  </tr>
                );
              })}
      ...
      {/* Add/Edit User Modal */}
      {showForm && (
        <Modal title={editingUser ? `Edit User — ${editingUser.name}` : "Tambah User Baru"} onClose={() => { setShowForm(false); setEditingUser(null); }} theme={T}>
          <div className="mms-form-grid">
            <FInput label="Nama Lengkap" value={form.name} onChange={v => setForm(f=>({...f,name:v}))} required placeholder="Nama lengkap" theme={T}/>
            <FInput label="Role" type="select" options={["admin","user"]} value={form.role} onChange={v => setForm(f=>({...f,role:v}))} theme={T}/>
            <FInput label="Username" value={form.username} onChange={v => setForm(f=>({...f,username:v}))} required placeholder="Username untuk login" theme={T}/>
            <FInput label="Password" value={form.password} onChange={v => setForm(f=>({...f,password:v}))} required placeholder="Password" theme={T}/>
          </div>

          {form.role !== "admin" && (
            <div style={{ marginTop:20 }}>
              <div style={{ color: T.text, fontWeight:600, fontSize:13, marginBottom:12 }}>🔐 Hak Akses per Halaman</div>
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {PAGE_LIST.filter(p => p.id !== "settings").map(p => (
                  <div key={p.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 14px", borderRadius:10, background: T.input, border:`1px solid ${T.inputBorder}` }}>
                    <span style={{ color: T.text, fontSize:13, fontWeight:500 }}>{p.label}</span>
                    <div style={{ display:"flex", gap:6 }}>
                      {PERM_OPTIONS.map(o => {
                        const active = (form.permissions[p.id] || "none") === o.val;
                        return (
                          <button key={o.val} onClick={() => setPermission(p.id, o.val)} style={{
                            padding:"4px 12px", borderRadius:999, fontSize:11, fontWeight:600, cursor:"pointer", border:"none",
                            background: active ? o.color : T.tableBorder,
                            color: active ? "#fff" : T.textMuted,
                            opacity: active ? 1 : 0.7,
                          }}>{o.val === "none" ? "❌" : o.val === "view" ? "👁 Lihat" : "✏️ Edit"}</button>
                        );
                      })}
                    </div>
                  </div>
          ...
          <div style={{ display:"flex", gap:10, marginTop:20, justifyContent:"flex-end" }}>
            <BtnSecondary onClick={() => { setShowForm(false); setEditingUser(null); }} theme={T}>Batal</BtnSecondary>
            <BtnPrimary onClick={saveUser}><Save size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>{editingUser ? "Update" : "Simpan"}</BtnPrimary>
          </div>
        </Modal>
      )}
  ...
  async function addUser() {
    if (!form.username || !form.password || !form.name) return alert("Lengkapi semua field!");
    if (users.some(u => u.username.toLowerCase() === form.username.trim().toLowerCase())) return alert("Username sudah dipakai!");
...
// ─── LOGIN PAGE ───────────────────────────────────────────────────────────────
function LoginPage({ users, onLogin, theme }) {
  const T = theme;
  ...
  return (
    <div style={{ background: T.bg, minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans','Nunito',system-ui,sans-serif", padding:20 }}>
      <form onSubmit={submit} style={{ width:"100%", maxWidth:380, background: T.card, backdropFilter: T.blur, WebkitBackdropFilter: T.blur, boxShadow: T.shadow, border:`1px solid ${T.cardBorder}`, borderRadius:20, padding:32 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", marginBottom:20 }}>
          <div style={{ width:52, height:52, borderRadius:14, display:"flex", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg,#f59e0b,#f97316)", boxShadow:"0 4px 14px rgba(245,158,11,0.4)" }}><Wrench size={24} color="#000" strokeWidth={2.2}/></div>
        </div>
        <h2 style={{ color: T.text, fontWeight:700, fontSize:20, margin:"0 0 4px", textAlign:"center" }}>Maintenance Management System</h2>
        <p style={{ color: T.textSub, fontSize:13, margin:"0 0 24px", textAlign:"center" }}>Masuk untuk melanjutkan</p>

        <div style={{ marginBottom:14 }}>
          <label style={{ display:"block", fontSize:12, color: T.textSub, marginBottom:6, fontWeight:600 }}>Username</label>
          <div style={{ position:"relative" }}>
            <User size={15} style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", color: T.textMuted }}/>
            <input value={username} onChange={e => setUsername(e.target.value)} placeholder="Masukkan username" autoFocus
              style={{ width:"100%", boxSizing:"border-box", padding:"10px 12px 10px 36px", borderRadius:10, border:`1px solid ${T.inputBorder}`, background: T.input, color: T.text, fontSize:14, outline:"none" }}/>
          </div>
        </div>

        <div style={{ marginBottom:8 }}>
          <label style={{ display:"block", fontSize:12, color: T.textSub, marginBottom:6, fontWeight:600 }}>Password</label>
          <div style={{ position:"relative" }}>
            <Lock size={15} style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", color: T.textMuted }}/>
            <input type={showPw ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Masukkan password"
              style={{ width:"100%", boxSizing:"border-box", padding:"10px 36px 10px 36px", borderRadius:10, border:`1px solid ${T.inputBorder}`, background: T.input, color: T.text, fontSize:14, outline:"none" }}/>
            <button type="button" onClick={() => setShowPw(s => !s)} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color: T.textMuted, display:"flex" }}>
        ...
        <button type="submit" style={{ width:"100%", padding:"11px", borderRadius:10, fontWeight:700, fontSize:14, color:"#000", background:"#f59e0b", border:"none", cursor:"pointer", marginTop:8 }}>
          Masuk
        </button>
      </form>
    </div>
...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState(() => {
    try { return JSON.parse(localStorage.getItem("mms_current_user") || "null"); } catch { return null; }
  });
  ...
  const visiblePages = getVisiblePages(currentUser);

  function handleLogin(user) {
    setCurrentUser(user);
    localStorage.setItem("mms_current_user", JSON.stringify(user));
    setPage("dashboard");
  }
  ...
  useEffect(() => {
    (async () => {
      try {
        const [dt, sp, ord, act, td, as, us] = await Promise.all([
          loadCollection("downtimes"), loadCollection("spareparts"),
          loadCollection("orders"),    loadCollection("activities"),
          loadCollection("todos"),     loadCollection("assetmesin"), loadCollection("users"),
        ]);
        ...
        if (us.length)  { setUsers(us); }    else { for (const x of SEED_USERS)        await saveDoc("users",      x.id, x); setUsers(SEED_USERS); }
        setTodos(td);
        setAssets(as);
        setOrders(ord);
  ...
  useEffect(() => {
    ...
    const allowed = getVisiblePages(currentUser).map(p => p.id);
    if (!allowed.includes(page)) setPage(allowed[0] || "dashboard");
  }, [currentUser, page]);
  ...
  if (!currentUser) return <LoginPage users={users} onLogin={handleLogin} theme={T}/>;
    ...
    <div style={{ background: T.bg, minHeight:"100vh", fontFamily:"'DM Sans','Nunito',system-ui,sans-serif", color: T.text }}>
        ...
        <div style={{ position:"fixed", inset:0, zIndex:50, background:"rgba(0,0,0,0.6)" }} onClick={() => setMenuOpen(false)}>
          <div style={{ height:"100%", width:240, padding:20, background: T.sidebar, backdropFilter: T.blur, WebkitBackdropFilter: T.blur, display:"flex", flexDirection:"column", gap:4 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ borderTop:`1px solid ${T.divider}`, marginTop:8, paddingTop:8 }}>
              <div style={{ padding:"6px 12px", color: T.textMuted, fontSize:12 }}>{currentUser.name} ({currentUser.role})</div>
              <button onClick={handleLogout} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, border:"none", cursor:"pointer", textAlign:"left", width:"100%", fontWeight:500, fontSize:13, background:"transparent", color:"#ef4444" }}>
                <LogOut size={16} strokeWidth={2}/><span>Logout</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div style={{ display:"flex" }}>
        ...
        <aside className="mms-sidebar" style={{ width:240, flexShrink:0, height:"100vh", position:"sticky", top:0, background: T.sidebar, backdropFilter: T.blur, WebkitBackdropFilter: T.blur, borderRight:`1px solid ${T.divider}`, flexDirection:"column" }}>
          ...
          {/* User info + Theme toggle di sidebar */}
          <div style={{ padding:16, borderTop:`1px solid ${T.divider}` }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10, padding:"8px 10px", borderRadius:10, background: T.input }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, minWidth:0 }}>
                <div style={{ width:28, height:28, borderRadius:"50%", background:"rgba(245,158,11,0.15)", color:"#f59e0b", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}><User size={14}/></div>
                <div style={{ minWidth:0 }}>
                  <div style={{ color: T.text, fontSize:12, fontWeight:600, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{currentUser.name}</div>
                  <div style={{ color: T.textMuted, fontSize:10, textTransform:"capitalize" }}>{currentUser.role}</div>
                </div>
              </div>
              <button onClick={handleLogout} title="Logout" style={{ background:"transparent", border:"none", cursor:"pointer", color:"#ef4444", display:"flex", flexShrink:0 }}><LogOut size={15}/></button>
            </div>
        ...
        <main style={{ flex:1, overflowY:"auto", minHeight:"100vh" }}>
          <div className="mms-main" style={{ maxWidth:1100, margin:"0 auto", padding:"24px 20px" }}>
            ...
            {page === "downtime"   && <DowntimePage  downtimes={downtimes}  setDowntimes={setDowntimes} toast={toast} theme={T} perm={getPerm(currentUser,"downtime")}/>}
            {page === "spareparts" && <SparePartsPage parts={parts}         setParts={setParts}         toast={toast} theme={T} perm={getPerm(currentUser,"spareparts")}/>}
            {page === "orders"     && <OrderPage      orders={orders}        setOrders={setOrders}       toast={toast} theme={T} perm={getPerm(currentUser,"orders")}/>}
            {page === "activity"   && <ActivityPage   activities={activities} setActivities={setActivities} toast={toast} theme={T} perm={getPerm(currentUser,"activity")}/>}
            {page === "todo"       && <TodoPage       todos={todos}           setTodos={setTodos}           toast={toast} theme={T} perm={getPerm(currentUser,"todo")}/>}
            {page === "assetmesin" && <AssetMesinPage assets={assets}         setAssets={setAssets}         toast={toast} theme={T} perm={getPerm(currentUser,"assetmesin")}/>}
            {page === "settings"   && <SettingsPage   toast={toast} theme={T} users={users} setUsers={setUsers}/> }
          </div>
        </main>
      </div>
    </div>
