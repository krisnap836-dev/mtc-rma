...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      // Flag: apakah app sudah pernah diinisialisasi sebelumnya
      const initialized = localStorage.getItem("mms_initialized");
      ...
      if (!initialized) {
        // Pertama kali buka → isi seed data, lalu tandai sudah diinisialisasi
        if (!dt.length)  { for (const x of SEED_DOWNTIMES)  await saveDoc("downtimes",  x.id, x); setDowntimes(SEED_DOWNTIMES); }
        else setDowntimes(dt);
        if (!sp.length)  { for (const x of SEED_PARTS)      await saveDoc("spareparts", x.id, x); setParts(SEED_PARTS); }
        else setParts(sp);
        if (!act.length) { for (const x of SEED_ACTIVITIES) await saveDoc("activities", x.id, x); setActivities(SEED_ACTIVITIES); }
        else setActivities(act);
        localStorage.setItem("mms_initialized", "1");
      } else {
        // Sudah pernah dibuka → load apa adanya, JANGAN timpa dengan seed
        setDowntimes(dt);
        setParts(sp);
        setActivities(act);
      }
