...
const SEED_USERS = [
  ...
  { id: "u2", username: "user",  password: "user123",  role: "user",  name: "User Lapangan" },
];
...
const PAGES = [
  ...
  { id: "downtime",  label: "Downtime",      icon: "⏱️", roles: ["admin","user"] },
  { id: "spareparts",label: "Spare Part",    icon: "🔩", roles: ["admin"] },
  { id: "orders",    label: "Order Part",    icon: "🛒", roles: ["admin"] },
  { id: "activity",  label: "Daily Activity",icon: "📋", roles: ["admin","user"] },
  { id: "todo",      label: "To-Do List",    icon: "✅", roles: ["admin"] },
...
// ─── DOWNTIME PAGE ────────────────────────────────────────────────────────────
function DowntimePage({ downtimes, setDowntimes, toast, theme, isAdmin }) {
  const T = theme;
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"space-between", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {isAdmin && <BtnSecondary onClick={downloadTemplate} theme={T}><FileText size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>Template</BtnSecondary>}
          {isAdmin && <BtnSecondary onClick={() => fileRef.current.click()} theme={T}><Upload size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>Import Excel</BtnSecondary>}
          ...
          {isAdmin && <DeleteAllButton items={downtimes} colName="downtimes" setItems={setDowntimes} toast={toast}/>}
        ...
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
            <thead style={{ background: T.tableHead }}>
              <tr>{(isAdmin ? ["Tanggal","Plant","Mesin","Kategori","Durasi","Deskripsi","Teknisi","Status","Aksi"] : ["Tanggal","Plant","Mesin","Kategori","Durasi","Deskripsi","Teknisi","Status"]).map(h => (
                <th key={h} style={{ textAlign:"left", padding:"12px 16px", color: T.textSub, fontSize:11, fontWeight:600, whiteSpace:"nowrap" }}>{h}</th>
              ))}</tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={isAdmin ? 9 : 8} style={{ padding:"40px 16px", textAlign:"center", color: T.textMuted }}>
                  <Inbox size={28} style={{margin:"0 auto 8px", opacity:0.5}}/>Tidak ada data
                </td></tr>
...
// ─── DAILY ACTIVITY ───────────────────────────────────────────────────────────
function ActivityPage({ activities, setActivities, toast, theme, isAdmin }) {
  const T = theme;
  const [showForm, setShowForm] = useState(false);
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"space-between", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          ...
          {isAdmin && <DeleteAllButton items={activities} colName="activities" setItems={setActivities} toast={toast}/>}
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {items.map(a => (
                <div key={a.id} style={{ background: T.card, backdropFilter: T.blur, WebkitBackdropFilter: T.blur, boxShadow: T.shadow, border:`1px solid ${T.cardBorder}`, borderRadius:14, padding:16, display:"flex", gap:14, alignItems:"flex-start" }}>
                  ...
                  {isAdmin && <button onClick={() => remove(a.id)} style={{ flexShrink:0, padding:"4px 10px", borderRadius:6, fontSize:12, color:"#ef4444", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", cursor:"pointer" }}><Trash2 size={14}/></button>}
                </div>
              ))}
            </div>
          </div>
...
function SettingsPage({ toast, theme, users, setUsers }) {
  ...
  const blank = { username:"", password:"", name:"", role:"user" };
  const [form, setForm] = useState(blank);
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={cardStyle}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
          <h3 style={{ color: T.text, fontWeight:600, fontSize:15, margin:0 }}><Users size={16} style={{display:"inline",verticalAlign:"-3px",marginRight:6}}/>Kelola User</h3>
          <BtnPrimary onClick={() => setShowForm(s => !s)}><UserPlus size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>Tambah User</BtnPrimary>
        </div>

        {showForm && (
          <div style={{ background: T.input, border:`1px solid ${T.inputBorder}`, borderRadius:12, padding:16, marginBottom:16 }}>
            <div className="mms-form-grid">
              <FInput label="Nama" value={form.name} onChange={v => setForm(f=>({...f,name:v}))} required placeholder="Nama lengkap" theme={T}/>
              <FInput label="Role" type="select" options={["admin","user"]} value={form.role} onChange={v => setForm(f=>({...f,role:v}))} theme={T}/>
              <FInput label="Username" value={form.username} onChange={v => setForm(f=>({...f,username:v}))} required placeholder="Username login" theme={T}/>
              <FInput label="Password" value={form.password} onChange={v => setForm(f=>({...f,password:v}))} required placeholder="Password login" theme={T}/>
            </div>
            <div style={{ display:"flex", gap:10, marginTop:16, justifyContent:"flex-end" }}>
              <BtnSecondary onClick={() => { setShowForm(false); setForm(blank); }} theme={T}>Batal</BtnSecondary>
              <BtnPrimary onClick={addUser}><Save size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:6}}/>Simpan</BtnPrimary>
            </div>
          </div>
        )}
        ...
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
            <thead>
              <tr>{["Nama","Username","Password","Role","Aksi"].map(h => (
                <th key={h} style={{ textAlign:"left", padding:"0 12px 10px 0", color: T.textMuted, fontSize:11, fontWeight:600 }}>{h}</th>
              ))}</tr>
            </thead>
            <tbody>
              {users.map(u => (
                <tr key={u.id} style={{ borderTop:`1px solid ${T.tableBorder}` }}>
                  <td style={{ padding:"10px 12px 10px 0", color: T.text, fontWeight:500 }}>{u.name}</td>
                  <td style={{ padding:"10px 12px 10px 0", color: T.textSub }}>{u.username}</td>
                  ...
                  <td style={{ padding:"10px 12px 10px 0" }}>
                    <span style={{ fontSize:11, padding:"2px 8px", borderRadius:999, background: u.role==="admin" ? "rgba(245,158,11,0.15)" : "rgba(59,130,246,0.15)", color: u.role==="admin" ? "#f59e0b" : "#60a5fa", textTransform:"capitalize" }}>{u.role}</span>
                  </td>
                  <td style={{ padding:"10px 0" }}>
                    <button onClick={() => removeUser(u)} style={{ padding:"4px 10px", borderRadius:6, fontSize:12, color:"#ef4444", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", cursor:"pointer" }}><Trash2 size={14} style={{display:"inline",verticalAlign:"-2px",marginRight:4}}/>Hapus</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const allowed = PAGES.filter(p => p.roles.includes(currentUser.role)).map(p => p.id);
  ...
  const visiblePages = PAGES.filter(p => p.roles.includes(currentUser.role));
      ...
      <div style={{ display:"flex" }}>
        ...
        <main style={{ flex:1, overflowY:"auto", minHeight:"100vh" }}>
          <div className="mms-main" style={{ maxWidth:1100, margin:"0 auto", padding:"24px 20px" }}>
            ...
            {page === "downtime"   && <DowntimePage  downtimes={downtimes}  setDowntimes={setDowntimes} toast={toast} theme={T} isAdmin={isAdmin}/>}
            {page === "spareparts" && isAdmin && <SparePartsPage parts={parts}         setParts={setParts}         toast={toast} theme={T}/>}
            {page === "orders"     && isAdmin && <OrderPage      orders={orders}        setOrders={setOrders}       toast={toast} theme={T}/>}
            {page === "activity"   && <ActivityPage   activities={activities} setActivities={setActivities} toast={toast} theme={T} isAdmin={isAdmin}/>}
            {page === "todo"       && isAdmin && <TodoPage       todos={todos}           setTodos={setTodos}           toast={toast} theme={T}/>}
            {page === "settings"   && isAdmin && <SettingsPage   toast={toast} theme={T} users={users} setUsers={setUsers}/>}
          </div>
        </main>
      </div>
    </div>
