...
// ─── WHATSAPP NOTIFIKASI ──────────────────────────────────────────────────────
function sendWhatsAppNotif(item) {
  const enabled = localStorage.getItem("mms_wa_enabled") !== "0";
  const number  = localStorage.getItem("mms_wa_number") || "";
  if (!enabled || !number) return false;
  const msg =
`🔧 *LAPORAN DOWNTIME BARU*

📅 Tanggal: ${item.date}
🏭 Mesin: ${item.machine}
${CAT_ICONS[item.category]||""} Kategori: ${item.category}
⏱️ Durasi: ${item.duration} jam
📝 Deskripsi: ${item.description||"-"}
👤 Teknisi: ${item.technician||"-"}
📌 Status: ${item.status}

_Dikirim otomatis dari MMS Dashboard_`;
...
function DowntimePage({ downtimes, setDowntimes, toast, theme }) {
  ...
  async function submit() {
    ...
    if (sendWhatsAppNotif(item)) {
      toast("Membuka WhatsApp untuk kirim notifikasi...");
    }
  }
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:16 }}>
        ...
        <StatCard label="Selesai" value={done} sub={`${total?((done/total)*100).toFixed(0):0}% completion`} color="#10b981" icon="✅" theme={T}/>
        <StatCard label="Belum Selesai" value={total-done} sub="Perlu dikerjakan" color="#f59e0b" icon="🕒" theme={T}/>
        <StatCard label="Terlambat" value={overdue} sub="Lewat deadline" color="#ef4444" icon="⚠️" theme={T}/>
      </div>
            ...
            <div key={t.id} style={{ ...cardStyle, padding:16, display:"flex", gap:14, alignItems:"flex-start", opacity: t.done ? 0.6 : 1 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", gap:8, marginBottom:4 }}>
                  ...
                  {isOverdue && <span style={{ fontSize:11, padding:"2px 8px", borderRadius:999, background:"rgba(239,68,68,0.15)", color:"#ef4444", fontWeight:600 }}>⚠️ Terlambat</span>}
                </div>
              ...
              <button onClick={() => remove(t.id)} style={{ flexShrink:0, padding:"4px 10px", borderRadius:6, fontSize:12, color:"#ef4444", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", cursor:"pointer" }}>🗑</button>
            </div>
          );
        })}
      </div>
...
function SettingsPage({ toast, theme }) {
  ...
  const cardStyle = { background: T.card, border:`1px solid ${T.cardBorder}`, borderRadius:16, padding:24 };

  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div>
        <h2 style={{ color: T.text, fontWeight:700, fontSize:22, margin:0 }}>Settings</h2>
        <p style={{ color: T.textSub, fontSize:13, margin:"4px 0 0" }}>Pengaturan notifikasi dan preferensi</p>
      </div>

      <div style={cardStyle}>
        <h3 style={{ color: T.text, fontWeight:600, fontSize:15, margin:"0 0 4px" }}>📲 Notifikasi WhatsApp</h3>
        <p style={{ color: T.textMuted, fontSize:12, margin:"0 0 16px" }}>
          Setiap kali ada laporan downtime baru, sistem akan membuka WhatsApp dengan pesan otomatis ke nomor di bawah ini. Pengaturan ini tersimpan per-device (browser).
        </p>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
          <button onClick={() => setEnabled(e => !e)} style={{
            ...
            background: enabled ? "#10b981" : T.inputBorder, transition:"background 0.2s"
          }}>
          ...
          <span style={{ color: T.text, fontSize:13, fontWeight:500 }}>{enabled ? "Notifikasi WA Aktif" : "Notifikasi WA Nonaktif"}</span>
        </div>

        <div style={{ maxWidth:360 }}>
          <FInput label="Nomor WhatsApp Tujuan" value={waNumber} onChange={setWaNumber} placeholder="Contoh: 6281234567890" theme={T}/>
          <div style={{ fontSize:11, color: T.textMuted, marginTop:6 }}>
            Format: kode negara tanpa "+" atau "0" di depan. Contoh nomor 081234567890 → tulis <b>6281234567890</b>
          </div>
        </div>

        <div style={{ display:"flex", gap:10, marginTop:20 }}>
          <BtnPrimary onClick={saveSettings}>💾 Simpan Pengaturan</BtnPrimary>
          <BtnSecondary onClick={testNotif} theme={T}>📤 Test Kirim Notif</BtnSecondary>
        </div>
      </div>

      <div style={cardStyle}>
        <h3 style={{ color: T.text, fontWeight:600, fontSize:15, margin:"0 0 8px" }}>ℹ️ Tentang</h3>
...
export default function App() {
  ...
  return (
    <div style={{ background: T.bg, minHeight:"100vh", fontFamily:"'DM Sans','Nunito',system-ui,sans-serif", color: T.text }}>
      ...
      {/* Mobile top bar */}
      <div className="mms-mobile-bar" style={{ alignItems:"center", justifyContent:"space-between", padding:"10px 16px", position:"sticky", top:0, zIndex:40, background: T.sidebar, borderBottom:`1px solid ${T.divider}` }}>
      ...
      <div style={{ display:"flex" }}>
        {/* Desktop sidebar */}
        <aside className="mms-sidebar" style={{ width:240, flexShrink:0, height:"100vh", position:"sticky", top:0, background: T.sidebar, borderRight:`1px solid ${T.divider}`, flexDirection:"column" }}>
